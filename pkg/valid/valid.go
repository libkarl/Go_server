// Package valid contains set of validation functions.
package valid
...
// RelativePath checks if string is a relative path i.e. it is not absolute path.
// Minimal valid path length is 3 characters.
// Unix hidden files are not supported, nor relative paths starting with "./".
func RelativePath(s string) bool {
	switch {
	case len(s) < 3:
		return false

	case strings.HasPrefix(s, string(filepath.Separator)):
		return false
...
// URL will parse url and tell you
